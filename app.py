...
class Mapping:
    ...
    @property
    def all_sites(self):
        """Returns a list of (enzyme, position) tuples for all sites"""
        flat_list = []
        for pos, enzymes in self.sites.items():
            for enz in enzymes:
                flat_list.append((enz, pos))
        return flat_list
...
def generate_custom_question():
    data = request.json
    min_l = data.get('minLen', 3000)
    max_l = data.get('maxLen', 5000)
    total_length = random.randrange(min_l // 10, max_l // 10) * 10
    
    # We use a fixed set of enzymes to ensure we can name them consistently
    available_enzymes = ["EcoRI", "BamHI", "HindIII", "PstI", "XhoI", "A", "B", "C", "D", "E", "F", "G", "F"]
    num_to_use = random.randint(data.get('minEnz', 3), data.get('maxEnz', 4))
    selected_enzymes = random.sample(available_enzymes, num_to_use)
    
    # Create the ground-truth map
    mock_map = Mapping(length=total_length)
    
    for enz in selected_enzymes:
        # 1 to 3 cuts per enzyme for manageable puzzles
        num_cuts = random.randint(1, 3)
        existing_positions = list(mock_map.sites.keys())
        
        for _ in range(num_cuts):
            # Constraint: Keep fragments > 100bp
            valid_pos = False
            attempts = 0
            while not valid_pos and attempts < 20:
                # 2. Pick positions ONLY in increments of 10
                pos = float(random.randrange(0, total_length, 10))
                
                # Ensure no two sites (of any enzyme) are within 100bp
                if not any(abs(pos - p) < 100 or abs(pos - p) > (total_length - 100) for p in existing_positions):
                    valid_pos = True
                    mock_map.add_site(enz, pos)
                    existing_positions.append(pos)
                attempts += 1

    # Generate the Digest Categories
    digests = {}

    def clean_frags(enz_set):
        frags = mock_map.simulate_digest(enz_set)
        # Convert to int since they are now guaranteed multiples of 10
        return sorted([int(f) for f in frags], reverse=True)

    # 1. Single Digests
    for enz in selected_enzymes:
        digests[enz] = clean_frags({enz})

    # 2. Double Digests
    for pair in combinations(selected_enzymes, 2):
        name = "/".join(sorted(pair))
        digests[name] = clean_frags(set(pair))

    # 3. Triple Digest
    if len(selected_enzymes) >= 3:
        triples = list(combinations(selected_enzymes, 3))
        for trio in random.sample(triples, min(len(triples), 1)):
            name = "/".join(sorted(trio))
            digests[name] = clean_frags(set(trio))

    return jsonify({
        "digests": digests,
        "total_length": total_length,
        "raw_sites": [{"enz": s[0], "pos": int(s[1])} for s in mock_map.all_sites] 
    })
